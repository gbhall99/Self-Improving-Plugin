...
1. Set `.self-improve/state.json` `status` to `"stopped"` so any in-flight `/self-improve:run` re-arm aborts. (This is the kill switch the loop's guardrails check before every cycle. Note: `"paused"` is the loop's own auto-halt after a failure streak — it resumes only when the user runs `/self-improve:run`; `"stopped"` is the deliberate user stop.)
...
5. Confirm the loop is stopped and remind the user they can review with `/self-improve:report` and restart anytime with `/self-improve:run`.
