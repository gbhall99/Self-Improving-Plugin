...
|---|---|---|---|---|---|---|---|
| SI-006 | Validate generated `.self-improve/` state (config.json, state.json) in the QA gate | tech-debt | Maintainer / J2,J3 | Validator checks config/state schema + required fields when present; passes on valid, fails (exit 1) on bad; CI + `make validate` cover it | S | 9 | done |
